...
const createNextStocks = () => {
  return getSicsOfUnsavedIndustryStockCrawls()
    .then(_.first)
    .then((sic) => {
      if(sic) {
        console.log(`create stocks for industry ${sic}`);
        return Stock.createStocks(sic)
          .then(() => sic)
      }
    })
};

const crawlUnexploredStocks = () => {
  return getSicsOfSavedIndustryStockCrawls()
    .then(_.last)
    .then((sic) => {
      if(_.isString(sic)) {
        return Stock.crawlUnexploredStock(sic)
      }
    })
    .then(() => {
      return createNextStocks();
    })
    .then((nextSic) => {
      if(_.isString(nextSic)) {
        return crawlUnexploredStocks();
      }
    })
};

const filterInteractiveStocks = (minEntries) => {
  return getSicsOfSavedIndustryStockCrawls()
    // .then((sics) => {
    //   return _.slice(sics, 0, sics.length-1);
    // })
    .then((sics) => {
      const interActives = sics.map(sic => Stock.filterInteractiveStocks(sic, minEntries));
      return Promise.all(interActives);
    })
};

const filterInteractiveNotRegisteredStocks = (minEntries) => {
  return filterInteractiveStocks(minEntries)
    .then(_.flatten)
    .then((stocks) => {
      return StockService.getStocksFromResources()
        .then((resources) => {
          return _.map(resources, 'cik')
        })
        .then((ciks) => {
          return _.filter(_.values(stocks), (stock) => {
            return _.includes(ciks, stock.cik) === false;
          })
        })
    })
};
...
module.exports = {
  ...
  crawlEntries,
  crawlTitles,
  crawlUnexploredStocks,
  extractSICsFromStocks,
  filterInteractiveStocks,
  filterInteractiveNotRegisteredStocks,
  getEntries,
  getListing,
  getSicsOfUnsavedIndustryStockCrawls,
