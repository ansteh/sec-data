...
const crawlIndustryTitle = (sic) => {
  const targetUrl = getCompanyPageUrlBySIC({ sic });
  console.log('targetUrl', targetUrl);

  return got(targetUrl)
    .then(response => response.body)
    .then(document => parseIndustryTitle(sic, document));
};

const parseIndustryTitle = (sic, document) => {
  const $ = cheerio.load(document);
  const content = $('.companyMatch');

  return {
    title: content.text().replace(`Companies for  SIC ${sic} -`, '').trim(),
  };
};
