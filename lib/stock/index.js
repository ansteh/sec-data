...
const crawlStock = (stock) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    .then(() => {
      return crawlAnnualFilings(stock.ticker, {
        type: forms.annual
      });
    })
    .then(() => downloadAnnualFiles(stock.ticker, forms.annual))
    .then(() => {
      return crawlQuarterlyFilings(stock.ticker, {
        type: forms.quarterly
      });
    })
    .then(() => downloadQuarterlyFiles(stock.ticker, forms.quarterly))
    .then(() => parseFilingFilesByTicker(stock.ticker, forms.annual))
    .then(() => parseFilingFilesByTicker(stock.ticker, forms.quarterly));
};
