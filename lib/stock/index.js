  ...
  if(_.isEmpty(stocks)) {
    ...
    return Service.hasUnparsedFilings(stock)
      .then((unparsed) => {
        ...
        const infiniteLoops = ['DAL', 'ADM', 'BBY', 'CBK', 'REGX', 'SDLP'];

        // TODO: fundamentals accounting error cannot find year (every time on another filing)
        const cannotFindYear = ['EBAY', 'MCD', 'P', 'PG'];

        const exceptions = [...infiniteLoops, ...cannotFindYear];
