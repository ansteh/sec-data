...
const getDateContext = (ticker) => {
  return readFilings(ticker, '10-Q')
    .then((fillings) => {
      return dates = _
        .chain(Filings.Parser.find('DocumentPeriodEndDate', fillings))
        .flatten()
        .uniqBy('contextRef')
        .value();
    })
...
const find = (ticker, entry, formType = '10-K') => {
  ...
  return readFilings(ticker, formType)
    ...
    .then((entries) => {
      ...
      return entries;
      // return getDateContext(ticker)
      //   .then((datesContext) => {
      //     _.forEach(entries, (entry) => {
      //       let context = _.find(datesContext, { contextRef: entry.contextRef });
      //       entry.date = _.get(context, '$t');
      //     });
      //
      //     return entries;
      //   });
...
const traverse = (scope, values, path = '') => {
  ...
  if(_.has(scope, 'contextRef')) {
    if(_.includes(scope.contextRef)) {
      matches.push(`${path}`);
    }
  }
