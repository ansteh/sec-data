...
const StockPrices = (historical) => {
  ...
  const last = () => {
    return _.last(historical);
  };

  const getLastDate = () => {
    return _.get(last(), 'date');
  };

  const getDaysDifferenceFromLastTo = (date) => {
    const fromDate = getLastDate();

    if(fromDate) {
      return moment(moment(date)).diff(fromDate, 'days');
    }
  };
...
const TreeStockPrices = (historical) => {
  ...
  return {
    ...
    getDaysDifferenceFromLastTo: prices.getDaysDifferenceFromLastTo,
    getLastDate: prices.getLastDate,
