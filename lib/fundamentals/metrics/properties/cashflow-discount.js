...
const { getEarnings } = require('../util.js');

const getDiscountCashflowAsCollection = (getDCF, filings, maxGrowthRate) => {
  const earningsPerShareDiluted = getEarnings(filings);
...
const getCashflowDiscountByMeanAsCollection = (filings, maxGrowthRate) => {
...
const getTrailingTwelveMonthsDiscountCashflow = (summary, date, maxGrowthRate) => {
  ...
  let earningsPerShareDiluted = getEarnings(annual); //_.get(annual, 'EarningsPerShareDiluted');
...
const getTrailingTwelveMonthsDiscountCashflowAsCollection = (summary, maxGrowthRate) => {
  const quarterly = _.get(summary, 'quarterly');
  const earningsPerShareDiluted = getEarnings(quarterly); //_.get(quarterly, 'EarningsPerShareDiluted');
...
module.exports = {
  ...
  getCashflowDiscountByMeanAsCollection,
