...
const create = ({
  ...
  const simulate = (path) => {
    ...
    _.forEach(timeline, (date) => {
      ...
      // const dayRate = getRate(start, date);
      // console.log(dayRate, invested + invested * dayRate * moment(date).diff(moment(start), 'days') / 365);
  ...
  const buyShares = (states, path, date, filter) => {
    ...
    if(filter) {
      candidates = filter(candidates, { date, findStateByPath, takeFacts });
    }
  ...
  const getRate = (start, end) => {
    return ((netWealth + cash - invested)/invested) * 365 / moment(end).diff(moment(start), 'days');
  };
