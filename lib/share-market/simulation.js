...
const create = ({
  ...
  const updateShares = (states, path) => {
    //console.log('missing prices for:');
    delete states['ESV'];
    delete states['FDP'];
    delete states['NPK'];
    ...
    _.forOwn(shares, (share, ticker) => {
      ...
      if(state) {
        if(_.has(state.entry, 'close')) {
          ...
          if(log) console.log(`update stock missing price for: ${ticker}`);
        }
      } else {
        if(log) console.log(`update stock state missing for: ${ticker}`);
      }
    });
  };
  ...
  const sellShares = (states, path, filter) => {
    ...
    _.forOwn(shares, (share, ticker) => {
      ...
      if(_.isUndefined(state.margin)) {
        ...
        // console.log(`Undefined margin. Sold ${ticker} for ${result}. count ${share.count}. price ${share.price}`);
      }
      ...
      if(_.isNumber(state.margin)) {
        if(state.margin < sellMargin) {
          ...
          toDeleteTickers.push(ticker);
          // console.log(`sold ${ticker} for ${result} due to margin. count ${share.count}. price ${share.price}`);
        }
      }
    });
  ...
  const buySharesOfCandidates = (path, candidates, stakeCash = true) => {
    _.forOwn(candidates, (candidate, ticker) => {
      ...
      if(count > 0) {
        ...
        if(log) console.log(`buy ${count} shares of ${ticker}. Paid total: ${cost}`);
  ...
  const buyShares = (states, path, date, filter) => {
    ...
    _.forOwn(states, (candidate, ticker) => {
      ...
      // if(metric && metric.margin && _.isNaN(metric.margin) === false) {
    ...
    if(log) console.log('candidates count', _.keys(candidates).length);
    ...
    _.forOwn(shares, (share, ticker) => {
      ...
      if(state) {
        if(_.has(state.entry, 'close')) {
          ...
          if(log) console.log(`missing price for: ${ticker} => ${share.price || 0}`);
          netWealth += share.count * (share.price || 0);
        }
      } else {
        if(log) console.log(`state missing for: ${ticker}`);
      }
    });
  };
