...
const getTimeline = (stocks) => {
  ...
  // const historicals = stock.historical.getData('2015-01-01');
  const historicals = stock.historical.getData('2013-10-18');
...
const create = (stocks) => {
  ...
  const getShareMarketSpreadsOfStocksAnnualGrowthRateGreaterThan = (rate) => {
    const tickers = _.keys(stocks);
    const spreads = _.map(tickers, getSpreadGrowthRateByTicker);

    return _.filter(spreads, (spread) => {
      return _.get(spread, 'optimal.annualGrowthRate') > rate;
    });
  };
  ...
  return {
    ...
    getShareMarketSpreadsOfStocksAnnualGrowthRateGreaterThan,
...
const createShareMarketTimeline = ({ market, scopes }) => {
  ...
  const extendBy = (extend, date) => {
    ...
    return extend(candidates, { date, findStateByPath, market });
  };
