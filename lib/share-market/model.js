...
const createShareMarketTimeline = ({ market, scopes }) => {
  ...
  const takeFacts = _.curry((path, ticker, date) => {
    const facts = scope[ticker][path];
    return PriceModel.takeByFormat(facts, 'endDate', date, 4);
  });
...
const create = (stocks) => {
  ...
  const getStockSpread = (ticker) => {
    const stock = getStock(ticker);
    return getSpreadBy(stock);
  };

  const getSpreadGrowthRateByTicker = (ticker) => {
    const spread = getStockSpread(ticker);
    return getSpreadGrowthRate(spread);
  };
