...
const createMoneyAccount = ({ baseCurrency, currencyExchange }) => {
  ...
  const getExchangeRatesByCurrencyCodes = async () => {
    ...
    return currencyExchange.getExchangeRatesByCurrencyCodes(currencyCodes);
  };
  ...
  const hasEnoughBalance = async ({ currency, value }) => {
    ...
    const valueInBaseCurrency = await currencyExchange.convert({
      amount: value,
