...
describe("Money account:", function () {
  ...
  it("Has enough balance returns true when you get money (negative value)", async function () {
    const { moneyAccount } = mockMoneyAccount();
  ...
  it("We can wipe money account", async function () {
    const { moneyAccount } = mockMoneyAccount();

    await moneyAccount.moveMoney({ currency: "USD", value: 100 });
    const totalAmountAfterDeposit = await moneyAccount.getTotalAmount("USD");
    expect(totalAmountAfterDeposit.value).toEqual(100);

    moneyAccount.clear();

    const totalAmountAfterClear = await moneyAccount.getTotalAmount("USD");
    expect(totalAmountAfterClear.value).toEqual(0);
  });
});
