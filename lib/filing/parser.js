...
const prepareContext = (entry, filing) => {
...
const defaultPrepare = (targets, filing) => {
  if(targets) {
    return targets.map(target => prepareContext(target, filing));
  }
};
...
const Mapping = {
  Dividends: {
    ...
    prepare: (dividends, filing) => {
      // return dividends;
      return defaultPrepare(dividends, filing);
    }
  },
