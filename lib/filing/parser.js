...
const prepareContext = ({ contextRef, $t }, filing) => {
  ...
  if(!context) {
    ...
    // let matches = _
    //   .keys(filing)
    //   .filter(key => _.includes(_.toLower(key), 'context'));
  }
...
const Mapping = {
  Dividends: {
    ...
    prepare: (dividends, filing) => {
      return dividends;
      // if(dividends) {
      //   return dividends.map(dividend => prepareContext(dividend, filing));
      // }
    }
  },
  ...
  EarningsPerShareDiluted: {
    ...
    prepare: (earnings, filing) => {
      if(earnings) {
        return earnings.map(earning => prepareContext(earning, filing));
      }
    }
  },
