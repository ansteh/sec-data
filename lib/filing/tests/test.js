...
const testParseFiling = () => {
  ...
  return Filing.Parser.parseFile(filingFile)
    .then((content) => {
      ...
      console.log('PropertyPlant', filter(keys, 'PropertyPlant'));

      // console.log('DepreciationDepletionAndAmortization', filter(keys, 'DepreciationDepletionAndAmortization'));
      // console.log('PropertyPlantAndEquipmentNet', filter(keys, 'PropertyPlantAndEquipmentNet'));
      //
      // console.log('us-gaap:DepreciationDepletionAndAmortization', content['us-gaap:DepreciationDepletionAndAmortization']);
      // console.log('us-gaap:PropertyPlantAndEquipmentNet', content['us-gaap:PropertyPlantAndEquipmentNet']);

      // console.log(filter(keys, 'Shares'));
...
const examineJSON = (sourceFile, needle) => {
  let document = fs.readFileSync(sourceFile);
  let json = JSON.parse(document);
  let keysTree = getKeys(json);

  needle = _.toLower(needle);

  let keys = _
    .chain(_.keys(keysTree))
    .filter(key => _.includes(key, 'us-gaap'))
    .filter((key) => {
      if(needle) {
        return _.includes(_.toLower(key), needle);
      }
  ...
  // console.log(json['us-gaap:EarningsPerShareBasicAndDiluted']);
  console.log(json['us-gaap:WeightedAverageNumberOfShareOutstandingBasicAndDiluted']);
};

// examineJSON(`${__dirname}/../../../resources/stocks/SDLP/filings/20-F_2017-04-27.json`, 'share');
//EarningsPerShareBasicAndDiluted

// examineJSON(`${__dirname}/../../../resources/stocks/AAOI/filings/10-K_2017-03-09.json`, 'earn');
